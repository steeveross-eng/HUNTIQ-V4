    ...
    async def get_full_weather(self, lat: float, lng: float) -> FullWeatherResponse:
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                # Use OneCall API 3.0 for comprehensive data
            ...
            raise ValueError(f"Weather service unavailable: {e}")
        ...
        wind_deg = current_data.get("wind_deg", 0)
        current = CurrentWeather(
            ...
            pressure=current_data.get("pressure", 1015),
            wind=WindData(
                speed=round(current_data.get("wind_speed", 0) * 3.6, 1),  # m/s to km/h
                gust=round(current_data.get("wind_gust", 0) * 3.6, 1) if current_data.get("wind_gust") else None,
                direction=wind_deg,
            ...
            condition=WeatherConditionDetail(
                main=current_data.get("weather", [{}])[0].get("main", "Unknown"),
                description=self._translate_condition(current_data.get("weather", [{}])[0].get("description", "")),
                icon=current_data.get("weather", [{}])[0].get("icon", "01d"),
                icon_url=self._get_icon_url(current_data.get("weather", [{}])[0].get("icon", "01d"))
            ),
            ...
            sun=SunData(
                sunrise=datetime.fromtimestamp(current_data.get("sunrise", now.timestamp()), tz=timezone.utc),
                sunset=datetime.fromtimestamp(current_data.get("sunset", now.timestamp()), tz=timezone.utc)
            )
        )
        ...
        for h in hourly_data:
            hourly.append(HourlyForecast(
                timestamp=datetime.fromtimestamp(h.get("dt", now.timestamp()), tz=timezone.utc),
                temperature=h.get("temp", 0),
                feels_like=h.get("feels_like", 0),
                humidity=h.get("humidity", 0),
                wind_speed=round(h.get("wind_speed", 0) * 3.6, 1),
                condition=WeatherConditionDetail(
                    main=h.get("weather", [{}])[0].get("main", "Unknown"),
                    description=self._translate_condition(h.get("weather", [{}])[0].get("description", "")),
                    icon=h.get("weather", [{}])[0].get("icon", "01d"),
                    icon_url=self._get_icon_url(h.get("weather", [{}])[0].get("icon", "01d"))
                ),
                precipitation_probability=h.get("pop", 0) * 100
            ))
        ...
        for d in daily_data:
            ...
            daily.append(DailyForecast(
                date=datetime.fromtimestamp(d.get("dt", now.timestamp()), tz=timezone.utc).strftime("%Y-%m-%d"),
                temperature=DailyTemperature(
                    min=temp.get("min", 0),
                    max=temp.get("max", 0),
                    morning=temp.get("morn", 0),
                    day=temp.get("day", 0),
                    evening=temp.get("eve", 0),
                    night=temp.get("night", 0)
                ),
                feels_like=DailyTemperature(
                    min=feels.get("morn", 0),
                    max=feels.get("day", 0),
                    morning=feels.get("morn", 0),
                    day=feels.get("day", 0),
                    evening=feels.get("eve", 0),
                    night=feels.get("night", 0)
                ),
                humidity=d.get("humidity", 0),
                pressure=d.get("pressure", 1015),
                wind_speed=round(d.get("wind_speed", 0) * 3.6, 1),
                condition=WeatherConditionDetail(
                    main=d.get("weather", [{}])[0].get("main", "Unknown"),
                    description=self._translate_condition(d.get("weather", [{}])[0].get("description", "")),
                    icon=d.get("weather", [{}])[0].get("icon", "01d"),
                    icon_url=self._get_icon_url(d.get("weather", [{}])[0].get("icon", "01d"))
                ),
                precipitation_probability=d.get("pop", 0) * 100,
                uv_index=d.get("uvi", 0),
                sun=SunData(
                    sunrise=datetime.fromtimestamp(d.get("sunrise", now.timestamp()), tz=timezone.utc),
                    sunset=datetime.fromtimestamp(d.get("sunset", now.timestamp()), tz=timezone.utc)
                )
            ))
        
        # Calculate hunting analysis
        score, activity, recommendations = self._calculate_hunting_score(data)
        moon_phase, moon_name, moon_illum = self._calculate_moon_phase()
        best_times = self._get_best_hunting_times(
            current_data.get("sunrise", int(now.timestamp())),
            current_data.get("sunset", int(now.timestamp()))
        )
