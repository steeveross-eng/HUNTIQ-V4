...
async def health_check():
    ...
    return HealthResponse(
        status="operational", engine="affiliate_engine", version="1.0.0",
...
async def record_click(product_id: str = Query(...), session_id: str = Query(...)):
    """Record an affiliate click and get redirect URL"""
    service = get_affiliate_service()
    try:
...
async def get_clicks(limit: int = Query(500, le=1000)):
    """Get all affiliate clicks"""
    service = get_affiliate_service()
    return await service.get_all_clicks(limit)

@router.post("/confirm/{click_id}")
async def confirm_sale(click_id: str, commission_amount: float = Query(...)):
